...
public:
    ...
    MicaliVazirani(map<int, vector<int>> neighbours)
    {
        for (auto mp : neighbours)
        {
            nodes.push_back({mp.first, 0});
            for (auto e : mp.second)
            {
                neighbors[{mp.first, 0}].push_back({e, 0});
                edges[{mp.first, 0}][{e, 0}]["use"] = UNUSED;
                edges[{mp.first, 0}][{e, 0}]["visit"] = UNVISITED;
            }
        }
    }
